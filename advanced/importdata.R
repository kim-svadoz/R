...
dfcsv2 <- cSplit(indt=dfcsv,
                 ...
                 sep=",")
dfcsv2
class(dfcsv2) #class : 내부 데이터 타입을 보기 위한 함수

#타입 변경
#as의 함수들을 이용해서 변경. as는 ~~~가 아닌것을 ~~로 변경한다는 의미
# => data.tabmle과 data.frame의 속성을 가지고 있는 dfcsv2를 dataframe으로 변경
dfcsv2 <- as.data.frame(dfcsv2)
dfcsv2
class(dfcsv2) # 타입을 확인
class(dfcsv2$num) # data.frame
class(dfcsv2$mydata_1)# integer
...
#Factor는 순서와 명목형
#1,2,3,..........와 같이 어떤 순서를 의미하는 경우 : 컬럼의 데이터가 순서형
...
dfxlsx #tibble <- data.frame과 비슷한 다른종류의 라이브러리
